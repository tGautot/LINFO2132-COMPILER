...
public class CodeGenerator implements Opcodes{
    ...
    private void generateVarAssign(ASTNodes.VarAssign va, MethodVisitor mv){
        ...
        /*else if(va.ref instanceof ASTNodes.ArrayAccess){
            ...
            /*if (!va.value.exprType.type.equals("int") && !va.value.exprType.type.equals("real") &&
                    !va.value.exprType.type.equals("bool")) {
                mv.visitInsn(owner.getOpcode(IASTORE));
            } else {
                if (aa.exprType.type.equals("real") && va.value.exprType.type.equals("int")) {
                    mv.visitInsn(I2F);
                    mv.visitInsn(FASTORE);
                } else if (va.value.exprType.type.equals("int")) {
                    mv.visitInsn(owner.getOpcode(IASTORE));
                } else if (va.value.exprType.type.equals("real")) {
                    mv.visitInsn(FASTORE);
                } else if (va.value.exprType.type.equals("bool")) {
